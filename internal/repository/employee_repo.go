...
func (r *EmployeeRepository) FindByEmail(email string) (*models.Employee, error) {
	var emp models.Employee
	err := r.db.Preload("Permissions").Where("email = ?", email).First(&emp).Error
	if err != nil {
		return nil, err
	}
	return &emp, nil
...
func (r *EmployeeRepository) FindByUsername(username string) (*models.Employee, error) {
	var emp models.Employee
	err := r.db.Preload("Permissions").Where("username = ?", username).First(&emp).Error
	if err != nil {
		return nil, err
	}
	return &emp, nil
...
// List returns paginated employees matching the filter, plus total count
...
// SetPermissions replaces the employee's permission associations
